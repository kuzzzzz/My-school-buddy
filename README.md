...
1. **Set Root Directory to `backend`** (important for monorepo deploys).
2. Build command: `npm install && npm run build`
3. Start command: `npm run start`
4. Set all backend env vars.
5. Provision managed PostgreSQL + Neo4j and wire env vars.

If you deploy from repository root, do **not** run the root `build` script for backend-only services because it also builds the frontend workspace.
Use `npm run build:backend` instead.
...
Use the included `render.yaml` to provision backend with the correct `rootDir` and commands.
